    ...
    public class ReviewDAO : IRepository<Review>
    {
        ...
        int IRepository<Review>.Insert(Review element)
        {
            using (SqlConnection con = DatabaseSingleton.GetInstance())
            {
                ...
                {
                    command.Parameters.Add("@customer_id", SqlDbType.DateTime).Value = element.Customer_id;
                    command.Parameters.Add("@reviewtext", SqlDbType.Float).Value = element.Reviewtext;
